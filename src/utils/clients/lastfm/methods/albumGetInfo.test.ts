...
import type { LastFmAlbum } from 'utils/types/album';
...
describe('the `albumGetInfo` function', () => {
  ...
  it('should return the mbid if it was included in the album object', async () => {
    ...
    expect((response.info as LastFmAlbum).mbid).toBe('bd2fef46-f003-477f-bfb0-6b4de66cfcea');
  });
  ...
  it('should return an undefined mbid if not specified in request', async () => {
    ...
    expect((response.info as LastFmAlbum).mbid).toBeUndefined();
  });
});
